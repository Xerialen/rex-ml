...
def main(argv=None):
    ...
    parser.add_argument("--qw_takeoff_air_frac", type=float, default=0.0,
                        help="reverse curriculum steg -1: andel takeoff-episoder "
                             "som rutt-spawnar ur bottens verifierade lyckade "
                             "bana (route_states_rq_so.json); betalar endast "
                             "shaping+completion")
    parser.add_argument("--qw_takeoff_multihop", action="store_true",
                        help="flerhoppsregimen (rotorsaksfixen): terminera vid "
                             "gropfall/fullbordan/markcamp i stället för första "
                             "landningen; klätterbonus av i takeoff-envs")
    parser.add_argument("--qw_completion_bonus", type=float, default=12.0,
                        help="flerhopp: engångsbonus vid målnära grundad "
                             "landning (z>=ref-24, <=192 u från landing_2d)")
