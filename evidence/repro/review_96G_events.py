#!/usr/bin/env python
"""Rekonstruktion av 9.6G-gate-eventen (analyst-review 12, 2026-08-03).

EVENT A: probe_ledge_96G.json ep6 — "quad→ring SO ramla"
EVENT B: probe_ra_96G.json ep5  — "ring→quad SO ramla" (RA-spawnad prob)
...
Kör:  cd ~/rex-ml && PYTHONPATH=. sim/.venv-sf/bin/python \
          evidence/repro/review_96G_events.py
"""
import json
import sys
...
def review(dump_fn, ep_i, want_hopp):
    ...
    for i in range(i0 + 1, i1 + 1):
        ...
        if pl is None and p[2] > jg.LEDGE_Z and onl:
            ...
            if g[i]:
                n_anchored += 1
                anch.append((i, round(tax, 3)))
        if p[2] <= PIT_Z and fall is None:
            fall = (i, p.copy(), dpit, perp, tax)
    ...
    print(f"masksampel {n_mask} (grundade {n_anchored}, tax "
          f"{[a[1] for a in anch]}); side_acc {side_acc:.1f} => massa "
          f"{abs(side_acc) * dt:.1f} u·s (krav >= {jg.SIDE_MIN_MASS_US})")
    if fall:
        print(f"gropfall i transiten: i={fall[0]} pos "
              f"({fall[1][0]:.0f},{fall[1][1]:.0f},{fall[1][2]:.0f}) "
              f"dPit {fall[2]:.1f} perp {fall[3]:.1f} tax {fall[4]:.3f}")
...
review("/home/benjamin-adm/dumps/probe_ledge_96G.json", 6, "quad→ring SO")
review("/home/benjamin-adm/dumps/probe_ra_96G.json", 5, "ring→quad SO")
